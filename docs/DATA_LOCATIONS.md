...
production, development, the GWS archive, and object storage. The Ansible
inventory is the source of truth for configured paths. Use the Operations
Dashboard and current host checks for live availability, capacity, or time
coverage.
...
| Object-store bucket | `s3://gamb2le-o` through configured remote `gamb2le-object-store` | Independent additive archive for raw data, products, model evaluation, and manifests. |
| Object inventory evidence | `/data/aurora/internal/object_store_manifests` | Atomic comparison reports, progress heartbeat, and bounded immutable history. |
| Archive health contract | `/data/aurora/internal/archive_status/health-v1.json` | Authoritative read-only status consumed by dashboards and alerts. |
| Cloud retention audit | `/data/aurora/internal/retention` | Signed permit and coordinator receipts. |
...
| ASFS logger | `aurora@100.124.55.22:/home/aurora/data/asfs/raw/crd` | `/project/aurora/raw/asfs/crd` | `/data/aurora/products/asfs_logger` | `/data/aurora/products/asfs_logger/asfs_logger.zarr` |
| ASFS fast sonic | `aurora@100.124.55.22:/home/aurora/data/asfs/raw/crd` | `/project/aurora/raw/asfs/crd` | `/data/aurora/products/asfs_fast_sonic` | `/data/aurora/products/asfs_fast_sonic/asfs_fast_sonic.zarr` |
...
Production archives raw data, selected products, model-evaluation data, and
mirror manifests independently to GWS and object storage. Development receives
its live data from production and does not own archive writes. Exact enabled
timers and current archive coverage are operational state; check them on the
production host or in Operations.

| Job | Local source | GWS destination | Object-store destination |
| --- | --- | --- | --- |
| `raw` | `/project/aurora/raw/` | `/gws/ssde/j25b/gamb2le/data/incoming/aurora-cloud/raw/` | `s3://gamb2le-o/data/incoming/aurora-cloud/raw/` |
...
UTC. Raw files become parity evidence at six hours old; core and WXcam products
at 30 hours; model-evaluation data at six hours; and manifests at two hours.
Younger product files are reported as pending uploads, not gaps. See
[Backups and Archive Services](ARCHIVE_SERVICES.md) for writer cadence,
exclusions, and verification semantics.
...
The transfer hosts are transport endpoints, not alternative archives. The
archive data live under the GWS paths shown above.
...
## Local Recovery And Non-Active Zarr Directories
...
products or independent long-term archives. They should not be treated as
canonical data locations or retention evidence unless doing a specific local
product recovery.
